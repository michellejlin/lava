...
if __name__ == '__main__':
	...
	for x in range(0, len(sample_path_list)):
		...
		if 1 == 1:
			...
			subprocess.call('awk -F \\\$\\\'\\t\\\' \\\'BEGIN {FS=OFS="\\t"}{gsub("0/0","0/1",\\\$10)gsub("0/0","1/0",\\\$11)gsub("1/1","0/1",\\\$10)gsub("1/1","1/0",\\\$11)}1\\\' ' + sample + '.vcf > ' + sample + '_p.vcf', shell=True)
			...
			if not ref_done:
				...
				subprocess.call('awk -v ref=' + control_fastq + ' -F \\'[\\t:,]\\' \\'{print ref,","\\\$6" "substr(\\\$9,3)","\\\$12","\\\$39+0","substr(\\\$9,3)","\\\$6","substr(\\\$8,3)","substr(\\\$8,3,1)" to "substr(\\\$8,length(\\\$8))","\\\$2","\\\$36",0"}\\' ' + new_dir + '/ref.txt > ' + new_dir + '/ref.csv', shell=True)
				...
				subprocess.call('samtools flagstat ' + control_fastq + '.bam | awk \\'NR==1{printf \\\$1","} NR==5{printf \\\$1","} NR==5{print substr(\\\$5,2)}\\' ' '>> ' + new_dir + '/reads.csv  2>> ' + new_dir + '/lava.log', shell=True)
			...
			subprocess.call('samtools flagstat ' + sample + '.bam | awk \\'NR==1{printf \\\$1","} NR==5{printf \\\$1","} NR==5{print substr(\\\$5,2)}\\' >> ' + new_dir + '/reads.csv 2>> ' + new_dir + '/lava.log', shell=True)
