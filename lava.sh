...
subprocess.call('export PATH=$PATH:/Users/uwvirongs/Downloads/annovar', shell=True) 
SNPEFFPATH='/Users/uwvirongs/snpEff/'
subprocess.call('export PATH="/Users/uwvirongs/downloads/annovar/:$PATH"', shell=True)
...
def process(ref_seq_gb, fastq):
	...
	subprocess.call('bwa index lava_ref.fasta 2> lava.log', shell=True)
	subprocess.call('bwa mem -M lava_ref.fasta ' + fastq + ' > aln.sam 2>> lava.log', shell=True)
	subprocess.call('samtools view -S -b aln.sam > aln.bam 2>> lava.log', shell=True)
	subprocess.call('samtools sort aln.bam -o aln.sorted.bam 2>> lava.log', shell=True)
	subprocess.call('bcftools mpileup -Ou -f lava_ref.fasta aln.sorted.bam | bcftools call -mv -Oz -o calls.vcf.gz 2>> lava.log', shell=True)
	subprocess.call('tabix calls.vcf.gz 2>> lava.log', shell=True)
	subprocess.call('cat lava_ref.fasta | bcftools consensus calls.vcf.gz > consensus.fasta', shell=True)
...
if __name__ == '__main__':
	...
	if args.f != None and args.g != None:
		...
		reference_fasta, reference_gff = process(args.q, control_fastq)
	else:
	...
	subprocess.call('bwa index ' + reference_fasta + ' 2> lava.log', shell=True)
	print('Done indexing.')

	subprocess.call('samtools faidx ' + reference_fasta + ' 2>> lava.log', shell=True)
	subprocess.call(GATK + ' CreateSequenceDictionary -R ' + reference_fasta + ' --VERBOSITY ERROR 2>> lava.log', shell=True)
	...
	for sample in sample_path_list:
		...
		subprocess.call('bwa mem -M -R \'@RG\\tID:group1\\tSM:' + sample + '\\tPL:illumina\\tLB:lib1\\tPU:unit1\' -p -t 6 -L [17,17] ' + 
			reference_fasta + ' ' + sample + ' > ' + sample + '.sam' + ' 2>> lava.log', shell=True)

		subprocess.call('java -jar ' + PICARD + ' SortSam INPUT=' + sample + '.sam OUTPUT=' + sample + 
			'.bam SORT_ORDER=coordinate VERBOSITY=ERROR 2>> lava.log', shell=True)

		if args.dedup:
			subprocess.call('java -jar ' + PICARD + ' MarkDuplicates INPUT=' + sample + '.bam OUTPUT=' + sample + 
				'_dedup.bam METRICS_FILE=metrics.txt VERBOSITY=ERROR 2>> lava.log',shell=True)
			subprocess.call('cat ' + name + '_dedup.bam > ' + name + '.bam' + ' 2>> lava.log', shell=True)

		subprocess.call('java -jar ' + PICARD + ' BuildBamIndex INPUT=' + sample + '.bam VERBOSITY=ERROR 2>> lava.log', shell=True)
		subprocess.call('echo sample\tposition\tcov > ' + sample + '.genomecov', shell=True)
		subprocess.call('bedtools genomecov -d -ibam ' + sample + '.bam >> ' + sample +'.genomecov' + ' 2>> lava.log', shell=True)
		subprocess.call('samtools mpileup -f ' + reference_fasta + ' ' + sample + '.bam > ' + sample + '.pileup' + ' 2>> lava.log', shell=True)

	subprocess.call('gff3ToGenePred ' + reference_gff + ' AT_refGene.txt -warnAndContinue -useName -allowMinimalGenes 2>> lava.log', shell=True)
	subprocess.call('../retrieve_seq_from_fasta.pl --format refGene --seqfile ' + reference_fasta + ' AT_refGene.txt --out AT_refGeneMrna.fa 2>> lava.log', shell=True)

	subprocess.call('mkdir ' + new_dir + '/db/', shell=True)
	subprocess.call('mv AT_refGeneMrna.fa ' + new_dir + '/db/', shell=True)
	subprocess.call('mv AT_refGene.txt ' + new_dir + '/db/', shell=True)
	# TODO: Re-write these in python 
	subprocess.call('echo "Sample,Amino Acid Change,Position,AF,Change,Protein,NucleotideChange,LetterChange,Syn,Depth,Passage" > ' + new_dir + '/merged.csv', shell=True)
	subprocess.call('grep "ID=transcript:" ' + reference_gff + ' | awk -F\'[\t;:]\' \'{print $12 "," $4 "," $5}\' | sort -t \',\' -k2 -n > ' + new_dir + '/proteins.csv', shell=True)
	...
	for sample in sample_path_list:
		if sample != control_fastq:
			subprocess.call('java -jar ' +  VARSCAN + ' somatic ' + control_fastq + '.pileup ' + sample + '.pileup ' + sample + 
				'.vcf --validation 1 --output-vcf 1 --min-coverage 2 2>> lava.log', shell=True)
			...
			subprocess.call('export PATH=$PATH:/Users/uwvirongs/Downloads/annovar;export PATH="/Users/uwvirongs/downloads/annovar/:$PATH";convert2annovar.pl -withfreq -format vcf4 -includeinfo ' + sample + '_p.vcf > ' + sample + '.avinput 2>> lava.log', shell=True)
			#annotates all mutations with codon changes
			subprocess.call('export PATH=$PATH:/Users/uwvirongs/Downloads/annovar;export PATH="/Users/uwvirongs/downloads/annovar/:$PATH";annotate_variation.pl -outfile ' + sample + ' -v -buildver AT ' + sample + '.avinput ' + new_dir + '/db/ 2>> lava.log', shell=True)

			if not ref_done:
				subprocess.call('awk -F":" \'($18+0)>=5{print}\' ' + sample + '.exonic_variant_function > ref.txt', shell=True)
				subprocess.call('grep "SNV" ref.txt > a.tmp && mv a.tmp ref.txt', shell = True)
				subprocess.call('awk -v ref=' + control_fastq + ' -F \'[\t:,]\' \'{print ref,","$6" "substr($9,3)","$12","$39+0","substr($9,3)","$6","substr($8,3)","substr($8,3,1)" to "substr($8,length($8))","$2","$36",0"}\' ref.txt > ref.csv', shell=True)
				subprocess.call('cat ref.csv >> ' + new_dir + '/merged.csv', shell=True)
				...
				subprocess.call('samtools flagstat ' + control_fastq + '.bam | awk \'NR==1{printf $1","} NR==5{printf $1","} NR==5{print substr($5,2)}\' '
					'>> ' + new_dir + '/reads.csv  2>> lava.log', shell=True)
				subprocess.call('echo sample	position	cov > ' + sample + '.genomecov', shell=True)
				subprocess.call('bedtools genomecov -d -ibam ' + sample + '.bam >> ' + sample + '.genomecov 2>> lava.log', shell=True)
			...
			subprocess.call('samtools flagstat ' + sample + '.bam | awk \'NR==1{printf $1","} NR==5{printf $1","} NR==5{print substr($5,2)}\' >> ' + new_dir + '/reads.csv 2>> lava.log', shell=True)
			...
			subprocess.call('awk -v name=' + sample + ' -v sample=$SAMPLE -F\'[\t:,]\' \'{print name","$6" "substr($9,3)","$12","$49+0","substr($9,3)","$6","substr($8,3)","substr($8,3,1)" to "substr($8,length($8))","$2","$46","sample}\' ' + sample + '.txt > ' + sample + '.csv', shell = True)
	...
	subprocess.call('pythonw ../genome_protein_plots.py ' + new_dir + '/merged.csv ' + new_dir + '/proteins.csv ' + new_dir + '/reads.csv ' + new_dir, shell=True)
